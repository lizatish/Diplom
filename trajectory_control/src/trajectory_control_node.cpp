...
void modesCallback(const nav_msgs::GridCells &data);
...
static nav_msgs::GridCells modes;
...
int main(int argc, char **argv){
  ...
  ros::Subscriber modes_sub = m.subscribe("/target_path_modes", 8, modesCallback);
  ros::Subscriber path_sub = m.subscribe("/target_path", 8, targetPathCallback);
...
const float VELOCITY_STEP = 0.005;
const float CRITICAL_DIST = 0.05;
const float VELOCITY_MAX = 0.8;
const float VELOCITY_BASE = 0.5;
float current_velocity = 0.5;
...
//    cout << "Target x: " << target_x << " y: " << target_y << endl;
    ...
    // Если расстояние достигнуто
      ...
      modes.cells.erase(modes.cells.begin());
    }
  }
