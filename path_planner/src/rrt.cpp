...
void RRT::cleanPath(){
  ...
  vector<Node*> new_nodeList;
  cout << "222" << endl;
  while (i < N){
    cout << i << " " << j << endl;
    if (i < j){
      cout << 100000 << endl;
      if (collisionCheck(nodeList[i]) and collisionCheck((nodeList[j]))){
        cout << 30000 << endl;
        new_nodeList.push_back(nodeList[j]);
        i = j;
        j = N;
        cout << 200000 << endl;
      }
...
nav_msgs::Path RRT::gen_final_course(int goalInd){
  ...
  while(nodeList[goalInd]->parent != -1){
    ...
    for(int i = 0; i < node->path_x.size(); i++){
      ...
      //        cout <<  p.pose.position.x << " " <<  p.pose.position.y << endl;
    }
    goalInd = node->parent;
  }
  return path;
...
bool RRT::collisionCheck(Node* node){
  ...
  for(int k = 0; k < node->path_x.size(); k++){
    // если будут проблемы, то в origin
